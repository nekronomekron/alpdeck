...
std::function<void(const Finished&)> finishedCallback;

// Reads a script through the virtual filesystem. Internal: the host is the
// only thing that loads scripts, and it must fail loudly on a short read.
bool readScript(const String& path, String& source) {
    String localPath;
...
bool isRunning() { return taskHandle != nullptr; }

String currentPath() { return activePath; }
