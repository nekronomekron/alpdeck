...
constexpr const char* kLogTag = "LuaHost";
...
// Allocates the host primitives. Returns false when allocation fails, in which
// case no script will ever run and the boot should not continue.
bool init();

// Starts a script. Returns false when one is already running, the file is
// missing, or the task could not be created. Non-blocking: the outcome of the
// script itself arrives via onFinished.
bool run(const String& path);

bool isRunning();
String currentPath();

// Asks a running script to unwind at its next VM hook. Cooperative: a script
// trapped in a C binding will not notice until it returns.
void requestStop();

// Fires on the main loop, never on the Lua task, so handlers can safely start
// the next script.
void onFinished(std::function<void(const Finished&)> callback);
